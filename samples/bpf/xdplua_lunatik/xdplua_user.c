...
static int xdp_attach_ebpf(lua_State *L)
{
	char lua_filename[MAXFILENAMELEN];
	struct bpf_object *obj;
	int prog_fd;
...
static int xdp_detach(lua_State *L)
{
	int err;
	const char *ifname;
...
static const luaL_Reg xdplualib[] = {
	{"attach_ebpf", xdp_attach_ebpf},
	{"detach", xdp_detach},
	{NULL, NULL}
};
